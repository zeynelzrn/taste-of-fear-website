...
// --- ÖZEL FORM BİLEŞENİ (CUSTOM GOOGLE FORM) ---
function CustomForm() {
  const [submitted, setSubmitted] = useState(false);

  // --- SENİN VERDİĞİN LİNKTEN ÇIKARILAN BİLGİLER ---
  ...
  const entry_TARIF = "entry.125698084"; // Malzeme
  const entry_HATA = "entry.396713294";  // Hata
      ...
      >
        <div className="grid md:grid-cols-2 gap-6">
          <div>
            ...
            <input type="text" name={entry_AD} required placeholder="Drakula..." 
              className="w-full bg-gray-900/80 border border-gray-700 rounded p-3 text-white focus:border-neon-green focus:outline-none focus:shadow-[0_0_15px_rgba(57,255,20,0.3)] transition-all" />
          </div>
          <div>
            <label className="block text-neon-green font-bold mb-2 font-mono text-sm tracking-widest">MEZAR ADRESIN (MAIL)</label>
            <input type="email" name={entry_MAIL} required placeholder="ornek@mezar.com" 
              className="w-full bg-gray-900/80 border border-gray-700 rounded p-3 text-white focus:border-neon-green focus:outline-none focus:shadow-[0_0_15px_rgba(57,255,20,0.3)] transition-all" />
          </div>
        </div>
        ...
        <div>
          ...
          <textarea name={entry_TARIF} rows="3" placeholder="Göz küresi çorbası..." 
            className="w-full bg-gray-900/80 border border-gray-700 rounded p-3 text-white focus:border-neon-green focus:outline-none focus:shadow-[0_0_15px_rgba(57,255,20,0.3)] transition-all"></textarea>
        </div>

        <div>
          <label className="block text-neon-green font-bold mb-2 font-mono text-sm tracking-widest">VARSA HATALI DURUM (Bug)</label>
          <textarea name={entry_HATA} rows="2" placeholder="Oyun şurada çöktü..." 
            className="w-full bg-gray-900/80 border border-gray-700 rounded p-3 text-white focus:border-neon-green focus:outline-none focus:shadow-[0_0_15px_rgba(57,255,20,0.3)] transition-all"></textarea>
        </div>

        <button type="submit" className="w-full bg-neon-green text-black font-horror text-2xl py-4 rounded hover:bg-white hover:scale-[1.02] transition-all duration-300 shadow-[0_0_20px_rgba(57,255,20,0.5)] mt-4">
          GÖNDER
        </button>
      </form>
    </>
    ...
    <div className="min-h-screen text-gray-200 font-tech selection:bg-neon-green selection:text-black relative overflow-x-hidden">
      ...
      <nav className="fixed w-full border-b border-neon-green/20 bg-gray-950/80 backdrop-blur-md z-50 py-4 transition-all duration-300">
        <div className="container mx-auto px-4 flex justify-between items-center">
          ...
          <h1 className="text-2xl md:text-3xl text-neon-green font-horror tracking-widest cursor-pointer hover:text-white transition drop-shadow-[0_0_10px_rgba(57,255,20,0.8)] z-50">
            TASTE OF FEAR
          </h1>
          ...
          <ul className="hidden md:flex space-x-8 text-sm font-bold">
            ...
            {/* SIRALAMA GÜNCELLENDİ: Önce İLETİŞİM, Sonra FORUM */}
          ...
          <div className="md:hidden z-50">
            <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="text-white hover:text-neon-green transition">
      ...
      {/* --- HERO SECTION --- */}
      <section id="game" className="min-h-screen flex flex-col justify-center items-center text-center px-4 pt-24 relative">
        ...
        <h2 className="text-6xl md:text-9xl font-horror text-transparent bg-clip-text bg-gradient-to-b from-neon-green to-green-900 mb-6 drop-shadow-[0_0_25px_rgba(57,255,20,0.5)] animate-bounce-slow leading-tight">
          TASTE OF FEAR
        </h2>
        
        <p className="text-lg md:text-3xl max-w-3xl text-gray-100 mb-10 border-l-8 border-neon-green pl-6 font-bold shadow-black drop-shadow-lg text-left md:text-center">
          "Midem doldu ama ruhum hala aç..." <br />
          <span className="text-neon-green text-base md:text-lg font-normal block mt-2">Yanlış malzemeyi seçersen, kazandaki sonraki et sen olursun.</span>
        </p>
        ...
        <div className="flex flex-col md:flex-row gap-4 md:gap-6 w-full md:w-auto px-4">
          ...
          <a href="#menu" className="w-full md:w-auto px-8 py-4 border-2 border-neon-green text-neon-green font-horror text-lg md:text-xl rounded hover:bg-neon-green hover:text-black transition-all duration-300 flex justify-center items-center gap-3 hover:shadow-[0_0_20px_rgba(57,255,20,0.4)]">
            <BookOpen size={24} /> MENUYE BAK
          </a>

          <a href="#forum" className="w-full md:w-auto px-8 py-4 border-2 border-gray-500 text-gray-300 font-horror text-lg md:text-xl rounded hover:border-white hover:text-white transition-all duration-300 flex justify-center items-center gap-3 bg-black/40 hover:bg-black/80">
             <MessageSquare size={24} /> FORUMA KATIL
          </a>
        </div>
      </section>

      {/* --- CHARACTERS SECTION --- */}
      <section id="characters" className="py-40 relative bg-gray-950/80 backdrop-blur-md border-y border-neon-green/20 overflow-visible">
        <div className="container mx-auto px-4">
          <div className="text-center mb-40">
            <h3 className="text-4xl md:text-6xl font-horror text-white mb-6 inline-flex items-center gap-4 drop-shadow-[0_5px_5px_rgba(0,0,0,0.8)]">
              <Users className="text-neon-green animate-pulse" size={48} /> KARANLIK MUSTERILER
            </h3>
            <p className="text-gray-400 text-lg md:text-xl max-w-2xl mx-auto pb-4 border-b-2 border-neon-green/30">
                Bu mutfağın müdavimleri biraz... farklı.
            </p>
          </div>
          ...
          <div className="flex flex-wrap justify-center gap-x-8 gap-y-24"> 
            {characters.map((char) => (
              <div 
                key={char.id} 
                className="group relative w-full md:w-[calc(50%-2rem)] lg:w-[calc(33.33%-2rem)] max-w-[380px] bg-gray-900 border border-gray-700 hover:border-neon-green rounded-3xl transition-all duration-300 mt-16 hover:shadow-[0_0_50px_rgba(57,255,20,0.4)] transform-gpu will-change-transform backface-hidden"
              >
                <div className="absolute inset-0 w-full h-full pointer-events-none" style={{ clipPath: 'inset(-500px -50px 10px -50px)' }}>
                  <div className="absolute -top-28 left-1/2 -translate-x-1/2 w-full flex flex-col items-center">
                     ...
                     <img 
                      ...
                      alt={char.name} 
                      loading="lazy"
                      className="h-[550px] w-auto max-w-none object-contain drop-shadow-[0_15px_15px_rgba(0,0,0,1)] filter brightness-[0.8] transform-gpu"
                    />
                  </div>
                </div>
                ...
                <div className="relative pt-52 pb-6 px-4 text-center z-10 rounded-3xl overflow-hidden transform-gpu">
                  ...
                  <p className="text-gray-300 font-mono text-xs leading-tight drop-shadow-md px-2">
                    {char.story}
                  </p>
                </div>
              </div>
      ...
      <section id="menu" className="py-24 relative">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            <h3 className="text-4xl md:text-5xl font-horror text-white mb-4 inline-flex items-center gap-4 drop-shadow-md">
              <Flame className="text-neon-green animate-pulse" size={48} /> LANETLI TARIFLER
            </h3>
            <p className="text-gray-300 text-lg max-w-2xl mx-auto">
              Mutfaktaki kadim kitaptan sızan yasaklı formüller.
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {recipes.map((cat, index) => (
              <div key={index} className="bg-gray-950/60 backdrop-blur-md p-6 md:p-8 rounded-xl border border-white/10 hover:border-neon-green hover:shadow-[0_0_30px_rgba(57,255,20,0.2)] transition-all duration-500 group">
                <h4 className="text-2xl md:text-3xl font-horror text-neon-green mb-6 border-b border-gray-700 pb-3 flex justify-between items-center">
                  {cat.category}
                  <Utensils size={24} className="text-gray-500 group-hover:text-neon-green transition-colors" />
                </h4>
                <ul className="space-y-5">
                  {cat.items.map((item, idx) => (
                    <li key={idx} className="flex flex-col group/item">
                      <span className="font-bold text-lg md:text-xl text-gray-200 group-hover/item:text-neon-green transition-colors duration-300">
                        {item.name}
                      </span>
                      <span className="text-sm text-gray-400 font-mono mt-1 pl-3 border-l-2 border-gray-600 group-hover/item:border-neon-green transition-all">
                        {item.ingredients}
                      </span>
                    </li>
                  ))}
      ...
      <section id="contact" className="py-24">
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="bg-gray-950/80 backdrop-blur-xl p-8 md:p-12 rounded-2xl border border-neon-green/30 shadow-[0_0_50px_rgba(0,0,0,0.8)]">
            ...
            {/* SİHİRLİ BİLEŞEN: CUSTOM GOOGLE FORM */}
            <CustomForm />
          </div>
        </div>
      </section>

      {/* --- FEATURES (ORTADA KALDI) --- */}
      <section className="py-20 bg-gray-950/80 backdrop-blur-sm border-y border-gray-800">
        <div className="container mx-auto px-4">
      ...
      {/* --- FORUM SECTION (EN AŞAĞI ALINDI) --- */}
      <section id="forum" className="py-24 bg-gray-950/90 backdrop-blur-md border-t border-neon-green/20">
        <div className="container mx-auto px-4 max-w-5xl">
          <div className="text-center mb-12">
            <h3 className="text-4xl md:text-5xl font-horror text-white mb-4 inline-flex items-center gap-4 drop-shadow-md">
              <MessageSquare className="text-neon-green animate-pulse" size={48} /> OLUMCUL TARTISMALAR
            </h3>
            <p className="text-gray-400 text-lg">
              Diğer aşçılarla konuş, tariflerini paylaş veya lanetli sırlar hakkında dedikodu yap.
            </p>
          </div>
      ...
      {/* --- FOOTER --- */}
      <footer className="py-8 bg-black text-center border-t border-gray-800 text-gray-600 text-xs font-tech tracking-widest">
